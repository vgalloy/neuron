...
public class StandardNeuron implements Neuron {
    ...
    public boolean apply(final boolean... input) {
        return aggregationFunction.activation(apply(aggregationFunction.toDoubleArray(input)));
    }
    ...
    public double[] train(final boolean expected, final boolean... input) {
        return train(function().toDouble(expected), function().toDoubleArray(input));
    }

    @Override
    public double[] train(final double expected, final double... input) {
        checkInput(input);

        final double result = compute(input);
        final double derived = aggregationFunction.applyDerived(result);
        final double diff = expected - aggregationFunction.apply(result);
        final double error = derived * diff;

        final ErrorOutput firstError = computeError(firstCoefficient, aggregationFunction.trueValue(), error);
        this.firstCoefficient += firstError.correction;
        final double[] coefficientCorrection = input.clone();
    ...
    protected ErrorOutput computeError(final double currentCoefficient, final double input, final double error) {
        final double errorPerInput = error * currentCoefficient * LEARNING_MULTIPLICATOR;
